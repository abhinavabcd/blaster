...
from blaster.base import route_handler, process_post_params,\
...
def send_session_message(sock, session_id, request_params=None, headers=None, post_data=None, user=None):
	...
	if(session_obj):     
		...
		return get_data("http://"+session["addr"]+"/send_message/"+session_id+"?auth_key="+get_internal_auth_key().decode(), post=post_data).read()
...
def after_new_session_created(sock,request_params=None, headers=None, post_data=None, user=None):
	...
	session_obj = json.loads(post_data.decode())
